...
class JitGenerator:
    ...
    def __generate_nodes__(self) -> jnp.ndarray:
        ...
        nodes: jnp.ndarray = jnp.arange(1, 2 * self.__data__.node_count + 1, step=2, dtype=self.__data__.data_type)
        nodes = jnp.multiply(nodes, jnp.pi / (2 * self.__data__.node_count))
        nodes = jnp.cos(nodes)

        return jax_utils.rescale_array_to_interval(nodes, jnp.array([-1, 1], dtype=self.__data__.data_type), self.__data__.interpolation_interval)
