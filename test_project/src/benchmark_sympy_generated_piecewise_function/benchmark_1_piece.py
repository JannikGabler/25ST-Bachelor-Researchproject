...
for _ in range(warmups):
    ...
    get_sympy_callable("cos(x)")(x_array).block_until_ready()
    get_sympy_callable_piecewise([((-1, 1), "cos(x)")])(x_array).block_until_ready()
...
start = time.perf_counter()
sympy_callable = get_sympy_callable("cos(x)")
end = time.perf_counter()
