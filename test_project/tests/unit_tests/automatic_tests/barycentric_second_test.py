...
from test_project.src.barycentric_second.barycentric_second import (
    compute_weights,
    barycentric_type2_interpolate,
)
...
class MyTestCase(unittest.TestCase):
    ...
    def test_exact_interpolation_points(self):
        ...
        for node, true_value in zip(interpolation_nodes, function_values):
            interpolated_value = barycentric_type2_interpolate(
                jnp.array([node]), interpolation_nodes, function_values
            )[0]
            self.assertTrue(
                jnp.isclose(interpolated_value, true_value, atol=1e-8),
    ...
    def test_quadratic_function_midpoint(self):
        ...
        interpolated_value = barycentric_type2_interpolate(
            jnp.array([evaluation_point]), interpolation_nodes, function_values
        )[0]
        self.assertTrue(
            jnp.isclose(interpolated_value, expected_value, atol=1e-6),
    ...
    def test_constant_function(self):
        ...
        interpolated_values = barycentric_type2_interpolate(
            evaluation_points, interpolation_nodes, function_values
        )
        self.assertTrue(
            jnp.allclose(interpolated_values, 3.0),
    ...
    def test_linear_function(self):
        ...
        self.assertTrue(
            jnp.allclose(interpolated_values, expected_values, atol=1e-6),
    ...
    def test_evaluation_at_all_nodes(self):
        ...
        interpolated_values = barycentric_type2_interpolate(
            interpolation_nodes, interpolation_nodes, function_values
        )
        self.assertTrue(
            jnp.allclose(interpolated_values, function_values, atol=1e-8),
    ...
    def test_interpolation_speed_various_sizes(self):
        ...
        for n in grid_sizes:
            ...
            print(
                f"Type 2: Interpolation time for {n} evaluation points (JAX): {elapsed:.4f} seconds ({elapsed_ms:.1f} ms)"
            )
    ...
    def test_compute_weights_speed(self):
        ...
        for n in node_counts:
            ...
            print(
                f"compute_weights: Time for {n} nodes: {elapsed:.4f} s ({elapsed_ms:.1f} ms)"
            )
