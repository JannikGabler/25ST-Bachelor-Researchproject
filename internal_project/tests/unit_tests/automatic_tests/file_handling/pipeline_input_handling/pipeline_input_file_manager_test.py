...
class LoadFromFile(unittest.TestCase):
    def test_load_with_all_attributes_set(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            pipeline_input_data: PipelineInputData = (
                PipelineInputFileManager.load_from_file(temp_file)
            )
            ...
            self.assertEqual(
                "jnp.array([-1, 1])", pipeline_input_data.interpolation_interval
            )
            self.assertEqual('"x**2 + 1"', pipeline_input_data.function_expression)
            self.assertEqual(
                "[((0,1), 'x'), ((1,2), 'x**2')]",
                pipeline_input_data.piecewise_function_expression,
            )
            self.assertEqual(
                "True", pipeline_input_data.sympy_function_expression_simplification
            )
            self.assertEqual(
                "lambda x: x**2 + 3", pipeline_input_data.function_callable
            )
            self.assertEqual(
                "jnp.array([0.0, 1.0, 4.0, 9.0])",
                pipeline_input_data.interpolation_values,
            )

            self.assertEqual(
                {"secret_token": '"abc123"'},
                pipeline_input_data.additional_directly_injected_values,
            )
            self.assertEqual(
                {"custom_param": "[1, 2, 3]"}, pipeline_input_data.additional_values
            )
    ...
    def test_load_empty_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                None, pipeline_input_data.sympy_function_expression_simplification
            )
    ...
    def test_dictionary_names_as_keys(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                {"additional_values": "5", "additional_directly_injected_values": "9"},
                pipeline_input_data.additional_values,
            )
