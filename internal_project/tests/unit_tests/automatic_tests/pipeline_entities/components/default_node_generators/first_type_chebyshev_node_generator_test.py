...
class MyTestCase(unittest.TestCase):
    def test_normal_case_1(self):
        ...
        expected_nodes = jnp.array(
            [
                jnp.float16("0.9510565163"),
                jnp.float16("0.5877852523"),
                jnp.float16("0"),
                jnp.float16("-0.5877852523"),
                jnp.float16("-0.9510565163"),
            ],
        ...
        self.assertTrue(
            jnp.array_equal(jnp.array([-1, 1]), pipeline_data.interpolation_interval)
        )
        self.assertIsNone(pipeline_data.function_values)

        self.assertTrue(
            JaxUtils.all_close_enough(expected_nodes, pipeline_data.nodes, 1e-01)
        )
    ...
    def test_transformed_interval(self):
        ...
        expected_nodes: jnp.ndarray = jnp.array(
            [
                jnp.float16("0.9122476926"),
                jnp.float16("0.2364101886"),
                jnp.float16("-0.9814069131"),
                jnp.float16("-2.5"),
                jnp.float16("-4.018593087"),
                jnp.float16("-5.236410189"),
                jnp.float16("-5.912247693"),
            ],
            dtype=jnp.float16,
        )
        ...
        self.assertTrue(
            jnp.array_equal(jnp.array([-6, 1]), pipeline_data.interpolation_interval)
        )
        self.assertIsNone(pipeline_data.function_values)
