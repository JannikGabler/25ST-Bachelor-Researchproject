...
class MyTestCase(unittest.TestCase):
    ...
    def test_all_attributes_correctly_set(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            data.node_count = "5"
            data.interpolation_interval = (
                "jax.numpy.array([-1, 1], dtype=jax.numpy.float32)"
            )
            data.function_expression = '"sin(x) * cos(x)"'
            data.piecewise_function_expression = (
                '[((-1, 0), "sin(x)"), ((0, 1), "cos(x)")]'
            )
            ...
            data.interpolation_values = (
                "jax.numpy.array([1, 2, 3, 4, 5], dtype=jax.numpy.float32)"
            )
            ...
            self.assertTrue(
                jnp.array_equal(
                    ...
                    pipeline_input.interpolation_interval,
                )
            )
            ...
            self.assertEqual(
                True, pipeline_input.sympy_function_expression_simplification
            )

            callable_input: jnp.ndarray = jnp.linspace(-50000, 50000, 1000)
            expected_callable_results: jnp.ndarray = jnp.sin(callable_input) * jnp.cos(
                callable_input
            )
            callable_results: jnp.ndarray = pipeline_input.function_callable(
                callable_input
            )
            self.assertTrue(
                jnp.array_equal(expected_callable_results, callable_results)
            )

            self.assertTrue(
                jnp.array_equal(
                    jnp.array([1, 2, 3, 4, 5], dtype=jnp.float32),
                    pipeline_input.interpolation_values,
                )
            )
            self.assertEqual({"test1": Tree(None)}, pipeline_input.additional_values)
            self.assertEqual(
                {"test2": math.pi}, pipeline_input.additional_directly_injected_values
            )
