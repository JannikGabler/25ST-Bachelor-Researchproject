...
class ChebyshevPoints2Case(unittest.TestCase):
    ...
    def setUp(self):
        ...
        temp_pipeline_configuration_file = Path(
            temp_dir.name + "/pipeline_configuration.ini"
        )
        ...
        pipeline_configuration_data: PipelineConfigurationData = (
            PipelineConfigurationFileManager.load_from_file(
        ...
        pipeline_input_data: PipelineInputData = (
            PipelineInputFileManager.load_from_file(temp_pipeline_input_file)
        )
    ...
    def test_base_input_output(self):
        report: PipelineComponentExecutionReport = (
            self._pipeline_manager_.get_component_execution_report("0")
        )

        instantiation_info: PipelineComponentInstantiationInfo = (
            report.component_instantiation_info
        )

        self.assertEqual("0", instantiation_info.component_name)
        self.assertEqual(
            ComponentRegistry.get_component("Base Input"), instantiation_info.component
        )
        ...
        self.assertTrue(
            jnp.array_equal(jnp.array([-1, 1]), output.interpolation_interval)
        )
    ...
    def test_function_expression_input_output(self):
        report: PipelineComponentExecutionReport = (
            self._pipeline_manager_.get_component_execution_report("1")
        )
        ...
        self.assertEqual(
            ComponentRegistry.get_component("Function Expression Input"),
            instantiation_info.component,
        )
        # self.assertEqual({'test_attribute': jnp.float32}, instantiation_info.component_specific_arguments)
        ...
        self.assertTrue(
            JaxUtils.all_close_enough(
                jnp.sin(10 * x_values), output.original_function(x_values), rtol=1e-04
            )
        )
    ...
    def test_chebyshev_2_node_generator_output(self):
        report: PipelineComponentExecutionReport = (
            self._pipeline_manager_.get_component_execution_report("2")
        )
        ...
        self.assertEqual(
            ComponentRegistry.get_component("Chebyshev2 Node Generator"),
            instantiation_info.component,
        )
        # self.assertEqual({'test_attribute': jnp.float32}, instantiation_info.component_specific_arguments)
        ...
        expected_nodes = jnp.cos(
            jnp.arange(0, 37, dtype=output.data_type) * (jnp.pi / 36)
        )

        self.assertIsNotNone(output.interpolation_nodes)
        self.assertTrue(
            JaxUtils.all_close_enough(
                expected_nodes, output.interpolation_nodes, atol=1e-07, rtol=1e-04
            )
        )
    ...
    def test_interpolation_values_evaluator_output(self):
        report: PipelineComponentExecutionReport = (
            self._pipeline_manager_.get_component_execution_report("3")
        )
        ...
        self.assertEqual(
            ComponentRegistry.get_component("Interpolation Values Evaluator"),
            instantiation_info.component,
        )
        # self.assertEqual({'test_attribute': jnp.float32}, instantiation_info.component_specific_arguments)
    ...
    def test_newton_interpolation_output(self):
        report: PipelineComponentExecutionReport = (
            self._pipeline_manager_.get_component_execution_report("4")
        )
        ...
        self.assertEqual(
            ComponentRegistry.get_component("Newton Interpolation"),
            instantiation_info.component,
        )
        # self.assertEqual({'test_attribute': jnp.float32}, instantiation_info.component_specific_arguments)
        ...
        expected_fraction_nodes = NodesTestUtils.chebyshev_2_nodes(
            (Fraction(-1), Fraction(1)), output.node_count
        )
        expected_nodes = jnp.array(
            [float(frac) for frac in expected_fraction_nodes], dtype=output.data_type
        )
        ...
        expected_divided_differences: list[Fraction] = (
            InterpolationTestUtils.calc_divided_differences(
                expected_fraction_nodes, expected_interpolation_values
            )
        )
        evaluation_fraction_array: list[Fraction] = [
            Fraction(i, 10) for i in range(-10, 11)
        ]
        evaluation_float_array: jnp.ndarray = jnp.array(
            [float(frac) for frac in evaluation_fraction_array], dtype=output.data_type
        )
        expected_fraction_values = EvaluationTestUtils.evaluate_newton_polynom(
            expected_divided_differences,
            expected_fraction_nodes,
            evaluation_fraction_array,
        )
        expected_values = jnp.array(
            [float(frac) for frac in expected_fraction_values], dtype=output.data_type
        )
        ...
        self.assertTrue(
            JaxUtils.all_close_enough(
                expected_values,
                output.interpolant.evaluate(evaluation_float_array),
                atol=1e-06,
                rtol=1e-04,
            )
        )
