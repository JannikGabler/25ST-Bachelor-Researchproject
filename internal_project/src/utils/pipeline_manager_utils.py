...
class PipelineManagerUtils:
    ...
    def create_input_for_node(
        ...
        if len(node.predecessors) == 0:
            data, additional_data = cls._create_new_input_for_entry_node_(
                node, pipeline
            )
        else:
            data, additional_data = cls._create_input_for_non_entry_node_(
                node, pipeline, reports
            )

        old_attributes: dict[str, object] = cls._insert_overridden_attributes_(
            node, data, additional_data
        )
        return data, additional_data, old_attributes

    @classmethod
    def reverse_attribute_overrides(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        ...
        for attribute_name, old_value in old_attributes.items():
            if (
                attribute_name
                not in node.value.component.component_meta_info.attributes_modifying
            ):
    ...
    #######################
    @classmethod
    def _create_new_input_for_entry_node_(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        pipeline: Pipeline,
    ) -> tuple[list[PipelineData], AdditionalComponentExecutionData]:

        index_of_old_node: int = (
            pipeline.pipeline_configuration.components.get_index_of_node_(node)
        )

        new_configuration: PipelineConfiguration = deepcopy(
            pipeline.pipeline_configuration
        )
        new_input: PipelineInput = deepcopy(pipeline.pipeline_input)
        new_own_node: DirectionalAcyclicGraphNode[
            PipelineComponentInstantiationInfo
        ] = new_configuration.components[index_of_old_node]

        additional_component_execution_data: AdditionalComponentExecutionData = (
            AdditionalComponentExecutionData(
                {}, new_configuration, new_input, new_own_node, {}
            )  # TODO: overridden attributes
        )
    ...
    @classmethod
    def _create_input_for_non_entry_node_(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        pipeline: Pipeline,
        reports: dict[int, PipelineComponentExecutionReport],
        ...
        node_mapping: dict[
            int, DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo]
        ] = {}
        for old_node in old_configuration.components.breadth_first_traversal():
            old_node_index: int = old_configuration.components.get_index_of_node_(
                old_node
            )
        ...
        for old_predecessor in node.all_predecessors_traversal():
            ...
            new_predecessor: DirectionalAcyclicGraphNode[
                PipelineComponentInstantiationInfo
            ] = node_mapping[id(old_predecessor)]
            new_reports[id(new_predecessor)] = deepcopy(report)

        pipeline_data_list: list[PipelineData] = [
            new_reports[id(pre)].component_output for pre in new_own_node.predecessors
        ]
        return pipeline_data_list, AdditionalComponentExecutionData(
            {}, new_configuration, new_input, new_own_node, new_reports
        )  # TODO: overridden attributes

    @classmethod
    def _insert_overridden_attributes_(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        data: list[PipelineData],
        additional_data: AdditionalComponentExecutionData,
        ...
        overridden_attributes: dict[str, object] = deepcopy(
            node.value.overridden_attributes
        )
        old_attributes: dict[str, object] = {}
        ...
        for attribute_name, attribute_value in overridden_attributes.items():
            old_value: object = cls._override_attribute_(
    ...
    @classmethod
    def _override_attribute_(
        cls, attribute_name: str, attribute_value: object, data: list[PipelineData]
    ) -> object:
        old_value: object

        if attribute_name != "additional_values" and any(
            field.name == attribute_name for field in fields(PipelineData)
        ):
            ...
            old_value = data[0].additional_values.get(
                attribute_name, cls.REMOVE_VALUE_MARKER
            )
