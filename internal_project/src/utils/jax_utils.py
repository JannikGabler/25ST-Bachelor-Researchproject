...
class JaxUtils:
    ...
    @staticmethod
    @jax.jit
    def rescale_array_to_interval(
        array: jnp.ndarray, old_interval: jnp.ndarray, new_interval: jnp.ndarray
    ) -> jnp.ndarray:
    ...
    def all_close_enough(
        ...
        if atol is None and rtol is None:
            raise InvalidArgumentException(
                "Either the absolut or relative tolerance must be specified."
            )
