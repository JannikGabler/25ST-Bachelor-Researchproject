...
class RelativeErrorPlotComponentUtils:
    ...
    def plot_data(cls,
        ...
        for i, absolut_errors in enumerate(absolut_errors_list):
            cls._plot_absolute_error_values_(
                template, plot_points, absolut_errors, pipeline_data[i].interpolant.name, i, y_limits
            )
    ...
    def _create_plot_data_(cls, pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) \
        ...
        absolut_errors_list: list[jnp.ndarray] = [
            cls._calc_relative_errors_(plot_points, original_function_values, data.interpolant, data.data_type)
    ...
    @classmethod
    def _plot_absolute_error_values_(cls,
