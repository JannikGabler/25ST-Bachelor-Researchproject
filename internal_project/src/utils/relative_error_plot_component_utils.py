...
class RelativeErrorPlotComponentUtils:
    """
    Utility helpers for generating relative error plots.
    """
    ...
    def plot_data(cls, pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) -> PlotTemplate:
        """
        Create the relative error plot for the given pipeline data.
        ...
        Returns:
            PlotTemplate: Configured plot visualizing the relative errors for each interpolant.
        """
        ...
        for i, absolut_errors in enumerate(absolut_errors_list):
            cls._plot_absolute_error_values_(template, plot_points, absolut_errors, pipeline_data[i].interpolant.name, i, y_limits)
    ...
    def _create_plot_data_(cls, pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) -> tuple[Array, list[Array], tuple[Array, Array]]:
        ...
        absolut_errors_list: list[jnp.ndarray] = [cls._calc_relative_errors_(plot_points, original_function_values, data.interpolant, data.data_type) for data in pipeline_data]
    ...
    @staticmethod
    def _calc_relative_errors_(plot_points: jnp.ndarray, original_function_values: jnp.ndarray, function: CompilableFunction, data_type: DTypeLike) -> jnp.ndarray:
    ...
    def _plot_absolute_error_values_(cls, template: PlotTemplate, plot_points: jnp.ndarray, absolute_errors: jnp.ndarray, function_name: str, function_index: int, y_limits: tuple[jnp.ndarray, jnp.ndarray]) -> None:
        ...
        for segment in connectable_segments:
            ...
            template.ax.plot(x_array, y_array, linewidth=RelativeErrorPlotComponentConstants.LINE_WIDTH, linestyle=line_style, color=color, label=label if not label_used else None)
            label_used = True
