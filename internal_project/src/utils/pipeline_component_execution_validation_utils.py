...
from exceptions.pipeline_execution_attribute_modified_exception import (
    PipelineExecutionAttributeModifiedException,
)
from exceptions.pipeline_execution_attribute_unmodified_exception import (
    PipelineExecutionAttributeUnmodifiedException,
)
...
class PipelineComponentExecutionValidationUtils:
    ...
    @classmethod
    def _validate_dynamic_node_component_pre_constraints_(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        pipeline: Pipeline,
        reports: dict[int, PipelineComponentExecutionReport],
        ...
        pre_dynamic_constraints: list[PreDynamicConstraint] = (
            node.value.component.component_meta_info.pre_dynamic_constraints
        )
        ...
        for constraint in pre_dynamic_constraints:
            if not constraint.evaluate(input_data[0], input_data[1]):
                raise PipelineConstraintViolationException(
                    ...
                    f"{repr(constraint.get_error_message())}",
                    node,
                    constraint,
                )
    ...
    @classmethod
    def _validate_dynamic_node_component_post_constraints_(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        pipeline: Pipeline,
        reports: dict[int, PipelineComponentExecutionReport],
        ...
        post_dynamic_constraints: list[PostDynamicConstraint] = (
            node.value.component.component_meta_info.post_dynamic_constraints
        )
        input_data = PipelineManagerUtils.create_input_for_node(node, pipeline, reports)
    ...
    @classmethod
    def _validate_node_component_changes_(
        cls,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        reports: dict[int, PipelineComponentExecutionReport],
    ) -> None:
        ...
        attributes_modifying: set[str] = (
            node.value.component.component_meta_info.attributes_modifying
        )
        attribute_names: list[str] = [
            field.name
            for field in fields(PipelineData)
            if field.name != "additional_values"
        ] + list(result_data.additional_values.keys())

        for attribute_name in attribute_names:
            if attribute_name in attributes_modifying:
                cls._validate_that_component_did_modify_attribute_(
                    attribute_name, node, reports
                )
            else:
                cls._validate_that_component_did_not_modify_attribute_(
                    attribute_name, node, reports
                )

    @classmethod
    def _validate_that_component_did_modify_attribute_(
        cls,
        attribute_name: str,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        reports: dict[int, PipelineComponentExecutionReport],
        ...
        direct_attribute_set: bool = (
            attribute_name != "additional_values"
            and hasattr(result_data, attribute_name)
            and getattr(result_data, attribute_name) is not None
        )

        additional_value_set: bool = (
            attribute_name in result_data.additional_values
    ...
    @classmethod
    def _validate_that_component_did_not_modify_attribute_(
        cls,
        attribute_name: str,
        node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo],
        reports: dict[int, PipelineComponentExecutionReport],
        ...
        if (
            node.value.component.component_meta_info.allow_additional_value_modifications_outside_specification
        ):
            return
        ...
        if node.predecessors:
            for predecessor in node.predecessors:
                ...
                if cls._is_attribute_equal_in_both_pipeline_datas_(
                    attribute_name, output_of_predecessor, result_data
                ):
                    return
        else:
            ...
            if cls._is_attribute_equal_in_both_pipeline_datas_(
                attribute_name, input_data, result_data
            ):
                return
    ...
    @classmethod
    def _is_attribute_equal_in_both_pipeline_datas_(
        cls,
        attribute_name: str,
        output_of_predecessor: PipelineData,
        result_pipeline_data: PipelineData,
    ) -> bool:
        if attribute_name in [field.name for field in fields(PipelineData)]:
            if isinstance(getattr(output_of_predecessor, attribute_name), jnp.ndarray):
                if jnp.array_equal(
                    getattr(output_of_predecessor, attribute_name),
                    getattr(result_pipeline_data, attribute_name),
                ):
                    return True
            elif getattr(output_of_predecessor, attribute_name) == getattr(
                result_pipeline_data, attribute_name
            ):
                return True
        else:
            ...
            if (
                attribute_name in output_of_predecessor.additional_values
