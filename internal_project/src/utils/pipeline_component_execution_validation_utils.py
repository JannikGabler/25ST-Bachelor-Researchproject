...
class PipelineComponentExecutionValidationUtils:
    """
    Utility helpers for validating pipeline component execution. This class is not meant to be instantiated.
    """
    ...
    ######################
    @classmethod
    def validate_before_node_execution(cls, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], pipeline: Pipeline, reports: dict[int, PipelineComponentExecutionReport]) -> None:
        """
        Validate all pre-execution dynamic constraints for the given node.

        Args:
            node (DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo]): The component node to validate.
            pipeline (Pipeline): The current pipeline.
            reports (dict[int, PipelineComponentExecutionReport]): Execution reports collected so far.

        Raises:
            PipelineConstraintViolationException: If a pre-execution dynamic constraint evaluates to False.
        """
    ...
    @classmethod
    def validate_after_node_execution(cls, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], pipeline: Pipeline, reports: dict[int, PipelineComponentExecutionReport]) -> None:
        """
        Validate attribute modifications and all post-execution dynamic constraints for the given node.
        ...
        Raises:
            PipelineExecutionAttributeUnmodifiedException: If a required attribute was not modified.
            PipelineExecutionAttributeModifiedException: If a non-allowed attribute was modified.
            PipelineConstraintViolationException: If a post-execution dynamic constraint evaluates to False.
        """
    ...
    @classmethod
    def _validate_dynamic_node_component_pre_constraints_(cls, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], pipeline: Pipeline, reports: dict[int, PipelineComponentExecutionReport]) -> None:
        ...
        for constraint in pre_dynamic_constraints:
            if not constraint.evaluate(input_data[0], input_data[1]):
                raise PipelineConstraintViolationException(f"The pre dynamic constraint {repr(constraint)} of the component " 
    ...
    @classmethod
    def _validate_dynamic_node_component_post_constraints_(cls, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], pipeline: Pipeline, reports: dict[int, PipelineComponentExecutionReport]) -> None:
        ...
        for constraint in post_dynamic_constraints:
            if not constraint.evaluate(input_data, output_data):
                raise PipelineConstraintViolationException(f"The post dynamic constraint {repr(constraint)} of the component "
                                                           f"{repr(node.value.component_name)} with id {repr(node.value.component.component_id)} was violated. "
                                                           f"The constraint provided the following reason:\n\n"
                                                           f"{repr(constraint.get_error_message())}", node, constraint)
    ...
    @classmethod
    def _validate_node_component_changes_(cls, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], reports: dict[int, PipelineComponentExecutionReport]) -> None:
        ...
        attribute_names: list[str] = [field.name for field in fields(PipelineData) if field.name != "additional_values"] + list(result_data.additional_values.keys())
    ...
    @classmethod
    def _validate_that_component_did_modify_attribute_(cls, attribute_name: str, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], reports: dict[int, PipelineComponentExecutionReport]) -> None:
        result_data: PipelineData = reports[id(node)].component_output
        direct_attribute_set: bool = (attribute_name != "additional_values" and hasattr(result_data, attribute_name) and getattr(result_data, attribute_name) is not None)
        additional_value_set: bool = (attribute_name in result_data.additional_values and result_data.additional_values[attribute_name] is not None)

        if not direct_attribute_set and not additional_value_set:
            node_value: PipelineComponentInstantiationInfo = node.value

            raise PipelineExecutionAttributeUnmodifiedException(f"The component {repr(node_value.component_name)} with the id {repr(node_value.component.component_id)} "
    ...
    @classmethod
    def _validate_that_component_did_not_modify_attribute_(cls, attribute_name: str, node: DirectionalAcyclicGraphNode[PipelineComponentInstantiationInfo], reports: dict[int, PipelineComponentExecutionReport]) -> None:
        result_data: PipelineData = reports[id(node)].component_output
        ...
        node_value: PipelineComponentInstantiationInfo = node.value
        raise PipelineExecutionAttributeModifiedException(f"The component {repr(node_value.component_name)} with the id {repr(node_value.component.component_id)} did "
    ...
    def _is_attribute_equal_in_both_pipeline_datas_(cls, attribute_name: str, output_of_predecessor: PipelineData, result_pipeline_data: PipelineData) -> bool:
        if attribute_name in [field.name for field in fields(PipelineData)]:
            ...
            if (attribute_name in output_of_predecessor.additional_values and attribute_name in result_pipeline_data.additional_values
