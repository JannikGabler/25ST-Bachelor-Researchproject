...
class InterpolantPlotComponentUtils:
    ...
    def plot_data(
        ...
        template: PlotTemplate = PlotTemplate(
            figsize=InterpolantsPlotComponentConstants.FIGURE_SIZE
        )
        ...
        data.functions = [pd.original_function] + [
            pd.interpolant for pd in pipeline_data
        ]
        ...
        data.function_names = [pd.original_function.name] + [
            pd.interpolant.name for pd in pipeline_data
        ]
        ...
        data.scatter_size = PlotUtils.scatter_size_for_equidistant_circles(
            InterpolantsPlotComponentConstants.AMOUNT_OF_INF_SCATTER_POINTS,
            InterpolantsPlotComponentConstants.FIGURE_SIZE[0],
        )
        ...
        for i, function_values in enumerate(data.function_values):
            segments_to_plot, segments_to_scatter = (
                cls._extract_indices_of_connectable_segments_(function_values)
            )
    ...
    @staticmethod
    def _calc_function_values_(
        data: _Data_, pipeline_data: list[PipelineData]
    ) -> jnp.ndarray:
        function_values: jnp.ndarray = jnp.empty(
            (
        ...
        v: jnp.ndarray = PlotUtils.evaluate_function(
            pipeline_data[0].original_function,
            pipeline_data[0].data_type,
    ...
    def _calc_y_limits_(original_function_values: jnp.ndarray) -> jnp.ndarray:
        ...
        difference = InterpolantsPlotComponentConstants.Y_LIMIT_FACTOR * (
            max_value - min_value
        )
    ...
    def _clean_up_function_values_(data: _Data_) -> None:
        for i, function_values in enumerate(data.function_values):
            ...
            cleaned_values = PlotUtils.clamp_function_values(
                cleaned_values, data.y_limits
            )
    ...
    def _calc_scatter_y_distance_(template: PlotTemplate, data: _Data_) -> jnp.ndarray:
        ...
        height_in_inches: float = (
            InterpolantsPlotComponentConstants.FIGURE_SIZE[1] / template.fig.dpi
        )

        numerator = data.y_limits[1] - data.y_limits[0]
        denominator = (
            100 * height_in_inches / scatter_y_distance_inch
            - 2 * data.amount_of_functions_to_plot
        )
    ...
    def _calc_borders_(data: _Data_, interval: jnp.ndarray) -> jnp.ndarray:
        ...
        borders = borders.at[1, 0].set(
            data.y_limits[0]
            - data.amount_of_functions_to_plot * data.scatter_y_distance
        )
        borders = borders.at[1, 1].set(
            data.y_limits[1]
            + data.amount_of_functions_to_plot * data.scatter_y_distance
        )
        return borders
    ...
    def _plot_interpolation_points_(
        template: PlotTemplate, pipeline_data: PipelineData
    ) -> None:
    ...
    def _plot_segments_(cls, template: PlotTemplate, data: _Data_) -> None:
        for function_index, segments in enumerate(data.connectable_segments):
            ...
            for plot_segment in segments[0]:
                ...
                y_values = jnp.array(
                    [
                        data.function_values[function_index][index]
            ...
            for scatter_segment in segments[1]:
                ...
                if len(scatter_segment) > 1:
                    ...
                    amount_of_intermediate_points = jnp.floor(
                        (right_segment_border - left_segment_border)
                        / data.scatter_x_distance
                    )
                    intermediate_points = (
                        left_segment_border
                        + jnp.arange(1, amount_of_intermediate_points + 1)
                        * data.scatter_x_distance
                    )
                ...
                actual_y_values = PlotUtils.clamp_function_values(
                    actual_y_values, data.y_limits
                )
                ...
                for x_value, actual_y_value in zip(scatter_x_values, actual_y_values):
                    ...
                    if jnp.isneginf(actual_y_value):
                        y_value = (
                            data.y_limits[0] - function_index * data.scatter_y_distance
                        )
                        template.ax.scatter(
                            x_value,
                            y_value,
                        ...
                        y_value = (
                            data.y_limits[1] + function_index * data.scatter_y_distance
                        )
                        template.ax.scatter(
                            x_value,
                            y_value,
    ...
    def _create_plot_parameters_(
        cls, function_index: int, data: _Data_
    ...
    def _set_legend_(cls, template: PlotTemplate, data: _Data_) -> None:
        custom_lines: list = [
            Line2D([0], [0], color="red", marker="o", markersize=8, linestyle="None")
        ]
        labels = ["Nodes"]
