...
class PlotUtils:
    ...
    ######################
    @staticmethod
    def create_plot_points(
        interval: jnp.ndarray, amount_of_evaluation_points: int
    ) -> jnp.ndarray:
        return jnp.linspace(
            interval[0], interval[1], amount_of_evaluation_points, dtype=jnp.float32
        )

    @staticmethod
    def create_exact_plot_points(
        interval: jnp.ndarray, amount_of_evaluation_points: int
    ) -> list[Fraction]:
    ...
    def evaluate_function(
        function: CompilableFunction, data_type: DTypeLike, plot_points: jnp.ndarray
    ) -> jnp.ndarray:
        compiled_function: CompiledFunction = function.compile(
            len(plot_points), data_type
        )
    ...
    @staticmethod
    def clamp_function_values(
        function_values: jnp.ndarray, y_limits: tuple[jnp.ndarray, jnp.ndarray]
    ) -> jnp.ndarray:
        clamped_values = jnp.where(
            function_values < y_limits[0], -jnp.inf, function_values
        )
        clamped_values = jnp.where(
            clamped_values > y_limits[1], jnp.inf, clamped_values
        )
        return clamped_values

    @staticmethod
    def adaptive_scatter_size(
        num_points: int, modifier: float = 1.0, min_size: float = 10.0
    ) -> float:
    ...
    @staticmethod
    def scatter_size_for_equidistant_circles(
        number_of_circles: int, length_of_axis_inch: float
    ) -> float:
    ...
    @staticmethod
    def scatter_distance_for_equidistant_circles(
        number_of_circles: int, length_of_axis_coords: jnp.ndarray
    ) -> jnp.ndarray:
        return length_of_axis_coords / number_of_circles

    @staticmethod
    def scatter_touch_distance(
        x_min: float,
        x_max: float,
        y_min: float,
        y_max: float,
    ...
    @classmethod
    def create_plot_data_from_function_values(
        cls,
        function_name: str,
        function_index: int,
        ...
        clamped_function_values = cls.clamp_function_values(
            cleaned_function_values, y_limits
        )

        connectable_segments, single_points = (
            cls._extract_connectable_segments_and_single_points_(
                function_index, plot_points, clamped_function_values, y_limits
            )
        )

        return FunctionPlotData(
            function_name, function_index, connectable_segments, single_points
        )

    @classmethod
    def create_plot_data_from_compilable_function(
        cls,
        ...
        return PlotUtils.create_plot_data_from_function_values(
            function.name, function_index, plot_points, function_values, y_limits
        )

    @staticmethod
    def create_plot_meta_info_str(
        ...
        pipeline_conf_str: str = (
            f"Conf.: {repr(pipeline_conf_name)}" if pipeline_conf_name else ""
        )
        pipeline_input_str: str = (
            f"Input: {repr(pipeline_input_name)}" if pipeline_input_name else ""
        )

        data_types_str: set[str] = {
            data.data_type.__module__ + "." + data.data_type.__name__
            for data in pipeline_data
        }
        node_counts_str: set[str] = {str(data.node_count) for data in pipeline_data}
        intervals_str: set[str] = {
            str(data.interpolation_interval) for data in pipeline_data
        }
        # node_count_str: str = str(pipeline_data[0].node_count)
        # interval_str: str = str(pipeline_data[0].interpolation_interval)

        return (
            f"{pipeline_conf_str}; {pipeline_input_str}; Dtypes: {data_types_str}; Node_counts: {node_counts_str}; "
    ...
    @classmethod
    def _extract_connectable_segments_and_single_points_(
        cls,
        function_index: int,
        plot_points: jnp.ndarray,
        function_values: jnp.ndarray,
        ...
        connectable_indices_segments, single_point_indices = (
            cls._extract_connectable_segments_and_single_points_indices_(
                function_values
            )
        )
        ...
        for segment_indices in connectable_indices_segments:
            segment: list[tuple[jnp.ndarray, jnp.ndarray]] = [
                (plot_points[index], function_values[index])
        ...
        for single_point_index in single_point_indices:
            ...
            y_offset: float = (
                function_index
                * 2
                * math.sqrt(PlotUtils.adaptive_scatter_size(len(plot_points)) / math.pi)
            )
            new_y_value: jnp.ndarray = (
                y_limits[0] - y_offset
                if jnp.isneginf(old_y_value)
                else y_limits[1] + y_offset
            )
    ...
    @staticmethod
    def _extract_connectable_segments_and_single_points_indices_(
        function_values: jnp.ndarray,
