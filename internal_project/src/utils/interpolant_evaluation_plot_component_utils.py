...
class InterpolantEvaluationPlotComponentUtils:
    ...
    def plot_data(cls, pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) -> None:
        ...
        cls._plot_original_function_(plot_points, pipeline_data[0].original_function, y_limits)

        for i, data in enumerate(pipeline_data):
            cls._plot_interpolant_values_(data, i)
    ...
    @classmethod
    def _plot_interpolant_values_(cls, pipeline_data: PipelineData, plot_index: int) -> None:
        ...
        plt.scatter(evaluation_points, interpolant_values, color=color,
            s=PlotUtils.adaptive_scatter_size(len(evaluation_points)), label=label, zorder=20, alpha=0.6)



    @classmethod
    def _create_plot_data_(cls, pipeline_data: list[PipelineData]) -> tuple[Array, tuple[Array, Array]]:
    ...
    @classmethod
    def _plot_original_function_(cls, plot_points: jnp.ndarray, function: CompilableFunction,
        ...
        line_width, color, z_order, label = cls._create_plot_parameters_(function_plot_data, 0)
        label_used: bool = False

        for segment in connectable_segments:
            x_array, y_array = zip(*segment)
            plt.plot(x_array, y_array, linewidth=line_width, color=color,
    ...
    def _finish_up_plot_(pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) -> None:
        ...
        plt.suptitle(f"Interpolant evaluation plot")
