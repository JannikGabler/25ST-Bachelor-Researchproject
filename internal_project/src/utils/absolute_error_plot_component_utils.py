...
class AbsoluteErrorPlotComponentUtils:
    """
    Utility class for creating plots of the absolute error of interpolants.
    """
    ...
    def plot_data(cls, pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) -> PlotTemplate:
        """
        Generate and display a plot of the absolute errors for all interpolants contained in the given pipeline data.

        Args:
            pipeline_data (list[PipelineData]): A list of pipeline data.
            additional_data (AdditionalComponentExecutionData): Additional execution data.

        Returns:
            PlotTemplate: Configured plot displaying the absolute errors.
        """
        ...
        for i, absolute_errors in enumerate(absolute_errors_list):
            cls._plot_absolute_error_values_(template, plot_points, absolute_errors, pipeline_data[i].interpolant.name, i, y_limits)
    ...
    #######################
    @classmethod
    def _create_plot_data_(cls, pipeline_data: list[PipelineData], additional_data: AdditionalComponentExecutionData) -> tuple[Array, list[Array], tuple[Array, Array]]:
        main_data: PipelineData = pipeline_data[0]
        interval: jnp.ndarray = main_data.interpolation_interval

        plot_points: jnp.ndarray = PlotUtils.create_plot_points(interval, AbsoluteErrorPlotComponentConstants.AMOUNT_OF_EVALUATION_POINTS)

        original_function_values: jnp.ndarray = PlotUtils.evaluate_function(main_data.original_function, jnp.float32, plot_points)

        absolut_errors_list: list[jnp.ndarray] = [cls._calc_absolute_errors_(plot_points, original_function_values, data.interpolant, data.data_type) for data in pipeline_data]
    ...
    @staticmethod
    def _calc_absolute_errors_(plot_points: jnp.ndarray, original_function_values: jnp.ndarray, function: CompilableFunction, data_type: DTypeLike) -> jnp.ndarray:
    ...
    @classmethod
    def _plot_absolute_error_values_(cls, template: PlotTemplate, plot_points: jnp.ndarray, absolute_errors: jnp.ndarray, function_name: str, function_index: int, y_limits: tuple[jnp.ndarray, jnp.ndarray]) -> None:
        PlotUtils.replace_nan_with_inf(absolute_errors)
        PlotUtils.clamp_function_values(absolute_errors, y_limits)

        function_plot_data: FunctionPlotData = (PlotUtils.create_plot_data_from_function_values(function_name, function_index, plot_points, absolute_errors, y_limits))
        ...
        for segment in connectable_segments:
            ...
            template.ax.plot(x_array, y_array, linewidth=AbsoluteErrorPlotComponentConstants.LINE_WIDTH, linestyle=line_style, color=color, label=label if not label_used else None)
            label_used = True
