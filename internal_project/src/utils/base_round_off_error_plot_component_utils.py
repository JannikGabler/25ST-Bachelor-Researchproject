...
class BaseRoundOffErrorPlotComponentUtils:
    ...
    def plot_data(
        ...
        set_error_callable: callable,
    ) -> PlotTemplate:
        data: BaseRoundOffErrorPlotComponentUtilsData = (
            BaseRoundOffErrorPlotComponentUtilsData()
        )
        template: PlotTemplate = PlotTemplate(
            figsize=BaseRoundOffErrorPlotComponentConstants.FIGURE_SIZE
        )
    ...
    #######################
    @staticmethod
    def _set_evaluation_points_(
        data: BaseRoundOffErrorPlotComponentUtilsData, pipeline_data: list[PipelineData]
    ) -> None:
        pd: PipelineData = pipeline_data[0]

        data.evaluation_points = PlotUtils.create_plot_points(
            pd.interpolation_interval,
            BaseRoundOffErrorPlotComponentConstants.AMOUNT_OF_EVALUATION_POINTS,
        )

        data.evaluation_points_exact = PlotUtils.create_exact_plot_points(
            pd.interpolation_interval,
            BaseRoundOffErrorPlotComponentConstants.AMOUNT_OF_EVALUATION_POINTS,
        )

    @staticmethod
    def _set_exact_interpolation_nodes_(
        data: BaseRoundOffErrorPlotComponentUtilsData, pipeline_data: list[PipelineData]
    ) -> None:
        pd: PipelineData = pipeline_data[0]

        data.interpolation_nodes_exact = [
            Fraction.from_float(node.astype(float).item())
            for node in pd.interpolation_nodes
        ]

    @staticmethod
    def _set_exact_interpolation_values_(
        data: BaseRoundOffErrorPlotComponentUtilsData, pipeline_data: list[PipelineData]
    ) -> None:
        pd: PipelineData = pipeline_data[0]

        data.interpolation_values_exact = [
            Fraction.from_float(value.astype(float).item())
            for value in pd.interpolation_values
        ]

    @staticmethod
    def _set_barycentric_weights_(
        data: BaseRoundOffErrorPlotComponentUtilsData, pipeline_data: list[PipelineData]
    ) -> None:
        pd: PipelineData = pipeline_data[0]
        ...
        for i in range(pd.node_count):
            ...
            for j in range(pd.node_count):
                if i != j:
                    denominator *= (
                        data.interpolation_nodes_exact[i]
                        - data.interpolation_nodes_exact[j]
                    )

            data.barycentric_weights_exact.append(1 / denominator)

    @staticmethod
    def _set_exact_interpolant_values_(
        data: BaseRoundOffErrorPlotComponentUtilsData, pipeline_data: list[PipelineData]
    ) -> None:
        pd: PipelineData = pipeline_data[0]
        ...
        for exact_evaluation_point in data.evaluation_points_exact:
            match_index: int | None = next(
                (
                    index
                    for index, value in enumerate(data.interpolation_nodes_exact)
                    if value == exact_evaluation_point
                ),
                None,
            )

            if match_index is not None:
                data.interpolant_values_exact.append(
                    data.interpolation_values_exact[match_index]
                )
            else:
                ...
                for j in range(pd.node_count):
                    value: Fraction = data.barycentric_weights_exact[j] / (
                        exact_evaluation_point - data.interpolation_nodes_exact[j]
                    )
    ...
    def _set_y_threshold_(
        ...
        y_threshold: object = additional_data.overridden_attributes.get(
            ...
            BaseRoundOffErrorPlotComponentConstants.DEFAULT_Y_THRESHOLD,
        )

        if not isinstance(y_threshold, float):
            raise TypeError(
                f"The attribute {repr(BaseRoundOffErrorPlotComponentConstants.Y_THRESHOLD_ATTRIBUTE_NAME)}"
    ...
    def _set_y_limit_(
        ...
        y_limit: object = additional_data.overridden_attributes.get(
            ...
            BaseRoundOffErrorPlotComponentConstants.DEFAULT_Y_LIMIT,
        )

        if not isinstance(y_limit, float):
            raise TypeError(
                f"The attribute {repr(BaseRoundOffErrorPlotComponentConstants.Y_LIMIT_ATTRIBUTE_NAME)}"
                f"for the component {additional_data.own_graph_node.value.component_name} must be a float."
            )
    ...
    @staticmethod
    def _clean_up_errors_(
        data: BaseRoundOffErrorPlotComponentUtilsData, pipeline_data: list[PipelineData]
    ) -> None:
        pd: PipelineData = pipeline_data[0]
        y_limits = (
            jnp.array(-jnp.inf, dtype=pd.data_type),
            jnp.array(data.y_limit, dtype=pd.data_type),
        )
    ...
    @classmethod
    def _set_scatter_size_and_distances_(
        cls,
        ...
        data.scatter_size = 9 * PlotUtils.scatter_size_for_equidistant_circles(
            BaseRoundOffErrorPlotComponentConstants.AMOUNT_OF_EVALUATION_POINTS,
            BaseRoundOffErrorPlotComponentConstants.FIGURE_SIZE[0],
        )
    ...
    @staticmethod
    def _calc_scatter_y_distance_(
        template: PlotTemplate, data: BaseRoundOffErrorPlotComponentUtilsData
    ) -> float:
        scatter_y_distance_inch: float = math.sqrt(data.scatter_size / math.pi) / 72
        height_in_inches: float = (
            BaseRoundOffErrorPlotComponentConstants.FIGURE_SIZE[1] / template.fig.dpi
        )

        numerator = data.y_limit
        denominator = (
            100 * height_in_inches / scatter_y_distance_inch
            - data.amount_of_functions_to_plot
        )
    ...
    def _set_border_(
        ...
        pd: PipelineData = pipeline_data[0]
        upper_y_border = (
            data.y_limit + data.amount_of_functions_to_plot * data.scatter_y_distance
        )
        data.border = (pd.interpolation_interval, (0, upper_y_border))

    @classmethod
    def _set_connectable_segments_(
        cls, data: BaseRoundOffErrorPlotComponentUtilsData
    ) -> None:
        data.connectable_segments = []
        for i, error in enumerate(data.round_off_errors):
            segments_to_plot, segments_to_scatter = (
                cls._extract_indices_of_connectable_segments_(error)
            )
            data.connectable_segments.append((segments_to_plot, segments_to_scatter))

    @staticmethod
    def _extract_indices_of_connectable_segments_(
    ...
    @classmethod
    def _plot_segments_(
        cls, template: PlotTemplate, data: BaseRoundOffErrorPlotComponentUtilsData
    ) -> None:
        for function_index, segments in enumerate(data.connectable_segments):
            y_value_for_inf: float = (
                data.y_limit + function_index * data.scatter_y_distance
            )
            line_width, line_style, color, alpha, z_order = (
                cls._create_plot_parameters_(function_index, data)
            )

            for plot_segment in segments[0]:
                x_values = jnp.array(
                    [data.evaluation_points[index] for index in plot_segment]
                )
                y_values = jnp.array(
                    [
                        data.round_off_errors[function_index][index]
                        for index in plot_segment
                    ]
                )
            ...
            for scatter_segment in segments[1]:
                x_values = jnp.array(
                    [data.evaluation_points[index] for index in scatter_segment]
                )
                ...
                template.ax.scatter(
                    ...
                    s=data.scatter_size,
                    color=color,
                    zorder=data.amount_of_functions_to_plot,
                )

    @classmethod
    def _create_plot_parameters_(
        cls, function_index: int, data: BaseRoundOffErrorPlotComponentUtilsData
    ) -> tuple[float, str, str, float, int]:
    ...
    def _create_line_styles_(data: BaseRoundOffErrorPlotComponentUtilsData) -> list:
        ...
        return line_styles
        # return ["-" for _ in range(data.amount_of_functions_to_plot)]

    @staticmethod
    def _draw_y_threshold_line_(
        template: PlotTemplate, data: BaseRoundOffErrorPlotComponentUtilsData
    ) -> None:
        template.ax.plot(
            data.border[0],
            [data.y_threshold, data.y_threshold],
            color="red",
            linestyle="--",
            linewidth=1,
            zorder=data.amount_of_functions_to_plot + 1,
        )
