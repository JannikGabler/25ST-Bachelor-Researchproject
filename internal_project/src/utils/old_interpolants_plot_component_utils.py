...
class OldInterpolantsPlotComponentUtils:
    ...
    @classmethod
    def _plot_interpolation_points_(cls, template: PlotTemplate, pipeline_data: PipelineData) -> None:
    ...
    @classmethod
    def _create_plot_data_(cls, pipeline_data: list[PipelineData]
        ...
        y_limits: tuple[jnp.ndarray, jnp.ndarray] = cls._calc_y_limits_(
            functions[0], plot_points, OldInterpolantsPlotComponentConstants.Y_LIMIT_FACTOR
        )
    ...
    def _plot_function_(cls,
        ...
        line_width, line_style, color, alpha, z_order, label = cls._create_plot_parameters_(
        ...
        for segment in connectable_segments:
            ...
            template.ax.plot(
                x_array, y_array,
                linewidth=line_width, linestyle=line_style, color=color, alpha=alpha,
        ...
        for point in single_points:
            template.ax.scatter(
                point[0], point[1],
                color=color, s=PlotUtils.adaptive_scatter_size(len(plot_points)),
