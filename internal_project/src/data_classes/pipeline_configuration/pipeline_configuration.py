...
from exceptions.prohibited_attribute_override_exception import (
    ProhibitedAttributeOverrideException,
)
...
from exceptions.pipeline_constraint_violation_exception import (
    PipelineConstraintViolationException,
)
...
class PipelineConfiguration:
    ...
    def get_component_node_by_component_name(
        ...
        raise NoSuchPipelineComponentError(
            f"There is no pipeline component in this pipeline configuration with the name {repr(component_name)}."
        )
    ...
    def __eq__(self, other):
        if not isinstance(other, PipelineConfiguration):  # Covers None
            ...
            return (
                ...
                and self._supported_program_version_
                == other._supported_program_version_
                and self._runs_for_component_execution_time_measurements_
                == other._runs_for_component_execution_time_measurements_
    ...
    def __hash__(self) -> int:  # Instances is immutable
        return hash(
            (
                self._name_,
                self._supported_program_version_,
                self._runs_for_component_execution_time_measurements_,
                self._components_,
            )
        )
    ...
    def _parse_input_data_(self, input_data: PipelineConfigurationData) -> None:
        eval_name_space: dict[str, Any] = (
            DynamicModuleLoader.get_module_namespace()
            | PipelineConfigurationConstants.PARSING_EVAL_NAMESPACE
        )
    ...
    def _parse_regular_input_values_(
        self, input_data: PipelineConfigurationData, eval_name_space: dict[str, Any]
    ) -> None:
    ...
    def _parse_single_regular_input_value_(
        self, field_name: str, field_value: str, eval_name_space: dict[str, Any]
    ) -> None:
        transformed_field_name: str = f"_{field_name}_"

        if field_value:
            parsed_value: Any = PythonEvalUtils.try_expression_evaluation(
                field_value, eval_name_space
            )
    ...
    def _parse_additional_input_values_(
        ...
        for key, value in input_data.additional_values.items():
            parsed_value: Any = PythonEvalUtils.try_expression_evaluation(
                value, eval_name_space
            )
            self._additional_values_[key] = parsed_value

    def _parse_components_value_(
        self, input_data: PipelineConfigurationData, eval_name_space: dict[str, Any]
    ) -> None:
        ...
        components = PythonEvalUtils.try_expression_evaluation(
            input_data.components, eval_name_space
        )
        ...
        self._components_ = (
            self._map_input_components_dag_into_internal_components_dag_(components)
        )
    ...
    def _validate_single_attribute_type(
        self, field_name: str, type_hints: dict[str, Any]
    ) -> None:
    ...
    def _validate_set_attribute_type_(
        self, field_name: str, field_value: Any, type_annotation: Any
    ) -> None:
    ...
    def _validate_non_set_attribute_type_(
        self, field_name: str, type_annotation: Any
    ) -> None:
    ...
    @staticmethod
    def _map_input_components_dag_into_internal_components_dag_(
        input_dag: DirectionalAcyclicGraph[tuple[str, str, dict[str, str]]],
    ) -> DirectionalAcyclicGraph[PipelineComponentInstantiationInfo]:
        value_mapping: Callable[
            [tuple[str, str, dict[str, str]]], PipelineComponentInstantiationInfo
        ] = PipelineConfiguration._map_input_dag_node_value_to_instantiation_info_
        return input_dag.value_map(value_mapping)

    @staticmethod
    def _map_input_dag_node_value_to_instantiation_info_(
        node_value: tuple[str, str, dict[str, str]],
    ) -> PipelineComponentInstantiationInfo:
        from pipeline_entities.pipeline.component_entities.component_registry.component_registry import (
            ComponentRegistry,
        )

        component_info: PipelineComponentInfo = ComponentRegistry.get_component(
            node_value[1]
        )
        ...
        if component_info:
            ...
            for attribute_name, attribute_value in node_value[2].items():
                value: object = PythonEvalUtils.try_expression_evaluation(
                    attribute_value,
                    PipelineConfigurationConstants.PARSING_EVAL_NAMESPACE,
                )
                evaluated_overridden_attributes[attribute_name] = value

            return PipelineComponentInstantiationInfo(
                node_value[0], component_info, evaluated_overridden_attributes
            )
        else:
            raise NoSuchPipelineComponentError(
    ...
    def _check_for_static_constraint_violations_(self) -> None:
        for node in self._components_:
            ...
            for (
                static_constraint
            ) in component_info.component_meta_info.static_constraints:
