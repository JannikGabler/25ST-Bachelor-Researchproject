...
class PlotComponent(PipelineComponent, ABC):
    ...
    def perform_action(self) -> PipelineData:
        if (
            self.PLOT_COMPONENT_CONSTANTS_CLASS is None
            or self.PLOT_COMPONENT_UTILS_CLASS is None
        ):
            raise Exception(
                "Plot components must specify PLOT_COMPONENT_UTILS_CLASS and PLOT_COMPONENT_CONSTANTS_CLASS."
            )

        template: PlotTemplate = self.PLOT_COMPONENT_UTILS_CLASS.plot_data(
            self._pipeline_data_, self._additional_execution_info_
        )
        if template is None:
            raise Exception(
                f"PlotTemplate of {self.__class__.__name__} is None!\n(object: {self.__repr__()})"
            )
