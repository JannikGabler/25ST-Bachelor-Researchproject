...
"""
Component meta information for the absolute round-off error plot component.
This component modifies the "plots" artifact group and allows overriding of the attributes data_type, y_threshold, and y_limit.
It requires the attributes data_type, node_count, interpolation_interval, interpolation_nodes, interpolation_values, and interpolant to be present 
in the pipeline data.
In addition, it does not allow multiple executions for time measurements.
"""
absolute_round_off_error_plot_component_meta_info: ComponentMetaInfo = (
    ComponentMetaInfo(
        attributes_modifying={"plots"},

        attributes_allowed_to_be_overridden={"data_type", BaseRoundOffErrorPlotComponentConstants.Y_THRESHOLD_ATTRIBUTE_NAME,
        ...
        static_constraints=[AttributeRequiredConstraint("data_type"), AttributeRequiredConstraint("node_count"),
                            AttributeRequiredConstraint("interpolation_interval"), AttributeRequiredConstraint("interpolation_nodes"),
                            AttributeRequiredConstraint("interpolation_values"), AttributeRequiredConstraint("interpolant")],
