...
from pipeline_entities.pipeline.component_entities.default_components.default_evaluation_components import (
    __register_default_evaluation_components__,
)
from pipeline_entities.pipeline.component_entities.default_components.default_input_components import (
    __register_default_input_components__,
)
from pipeline_entities.pipeline.component_entities.default_components.default_interpolation_cores import (
    __register_default_interpolation_cores__,
)
from pipeline_entities.pipeline.component_entities.default_components.default_node_generators import (
    __register_default_node_generators__,
)
from pipeline_entities.pipeline.component_entities.default_components.default_plotting_components import (
    __register_default_plotting_components__,
)
from pipeline_entities.pipeline.component_entities.default_components.default_test_components import (
    __register_default_test_components__,
)
...
class ComponentRegistry:
    ...
    def __init__(self):
        raise NotInstantiableError(
            "The class 'ComponentRegistry' can not be instantiated."
        )
    ...
    def register_component(component_info: PipelineComponentInfo) -> None:
        if component_info is None:
            ...
            raise NoneError(
                f"A component with the ID '{component_info.component_id}' is already registered."
            )

        key: str = component_info.component_id.lower()
        ComponentRegistry.__dictionary__[key] = (
            component_info  # ComponentInfo is immutable
        )
