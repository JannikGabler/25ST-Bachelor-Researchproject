...
from pipeline_entities.pipeline.component_entities.component_meta_info.defaults.node_generators.equidistant_node_generator_meta_info import equidistant_node_generator_meta_info
...
class EquidistantNodeGenerator(NodeGenerator):
    """
    Pipeline component that generates equidistant interpolation nodes over the specified interval.
    """
    ...
    def __init__(self, pipeline_data: list[PipelineData], additional_execution_data: AdditionalComponentExecutionData) -> None:
        """
        Initialize the equidistant node generator and compile a JAX function to efficiently compute equidistant nodes.
    ...
    def perform_action(self) -> PipelineData:
        """
        Generate equidistant interpolation nodes and store them in the pipeline data.
    ...
    def _create_compiled_callable_(data_type: type, node_count: int, interpolation_interval: jnp.ndarray) -> callable:
        ...
        def _internal_perform_action_() -> jnp.ndarray:
            return jnp.linspace(interpolation_interval[0], interpolation_interval[1], node_count, dtype=data_type)
