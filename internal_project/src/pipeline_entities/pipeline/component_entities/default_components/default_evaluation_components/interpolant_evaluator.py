...
@pipeline_component(
    id="interpolant evaluator",
    type=EvaluatorComponent,
    meta_info=interpolant_evaluator_meta_info,
)
class InterpolantEvaluator(EvaluatorComponent):
    ...
    def __init__(
        ...
        amount_of_evaluation_points: int = len(
            self._pipeline_data_[0].interpolant_evaluation_points
        )
    ...
    def perform_action(self) -> PipelineData:
        ...
        interpolant_evaluation_points: jnp.ndarray = (
            data.interpolant_evaluation_points.astype(data.data_type)
        )

        interpolant_values: jnp.ndarray = self._compiled_interpolant_.evaluate(
