    ...
    def __init__(cls, name, bases, namespace, **kwargs):
        ...
        if not inspect.isabstract(cls):
            ...
            actual_parameter_types = [
                hints.get(parameter.name, None) for parameter in parameters
            ]

            if len(parameters) != len(expected_parameter_types) or any(
                actual_type != expected_type
                for actual_type, expected_type in zip(
                    actual_parameter_types, expected_parameter_types
                )
            ):

                raise TypeError(
                    f"The subclass '{cls.__name__}' of PipelineComponent has a constructor with invalid parameters. "
