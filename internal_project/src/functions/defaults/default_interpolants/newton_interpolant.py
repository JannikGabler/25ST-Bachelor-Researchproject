...
class NewtonInterpolant(CompilableFunction):
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, self.__class__):
            ...
            return (
                jnp.array_equal(
                    self._divided_differences_,
                    other._divided_differences_,
                    equal_nan=True,
                ).item()
    ...
    def _internal_evaluate_(self, evaluation_points: jnp.ndarray) -> jnp.ndarray:
        ...
        divided_differences: jnp.ndarray = self._divided_differences_.astype(
            self._data_type_
        )

        initial_accumulator: jnp.ndarray = jnp.zeros_like(
            evaluation_points
        ...
        def horner_step(i, val):
            reverse_index = n - 1 - i
            return (
                val * (evaluation_points - nodes[reverse_index])
                + divided_differences[reverse_index]
            )
