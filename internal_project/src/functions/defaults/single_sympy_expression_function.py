...
class SingleSympyExpressionFunction(CompilableFunction):
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}(function_expression={repr(self._function_expression_)}, "
                f"simplify_expression={repr(self._simplify_expression_)})")

    def __str__(self) -> str:
        return (f"{self.__class__.__name__}(function_expression={str(self._function_expression_)}, "
    ...
    def _create_jax_lambda_(self) -> callable:
        x = sympy.symbols("x")
        expr: Expr = sympy.sympify(self._function_expression_, evaluate=self._simplify_expression_) # Although PyCharm shows an error, this is correct
